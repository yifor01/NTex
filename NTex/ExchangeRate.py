...
class NTex(object):
    ...
    def gold(self, show=True):
        ...
        end = datetime.date(datetime.datetime.now().year,
                            datetime.datetime.now().month + 1, 1)
        if file:
            gold_data = pd.read_pickle(file[0])
            start = datetime.date(
                gold_data['日期'].max().year, gold_data['日期'].max().month, 1)
            os.remove(file[0])
        else:
        ...
        while start != end:
            print(start)
            tmp = pd.read_csv(
                f'https://rate.bot.com.tw/gold/csv/{start.year}-{start.month:02d}/TWD/0')
            tmp['日期'] = pd.to_datetime(
                tmp['日期'].astype('int'), format='%Y%m%d')
            if len(tmp.index):
        ...
        gold_data = gold_data.drop_duplicates().sort_values(
            '日期').reset_index()[tmp.columns]
        gold_data.to_pickle(
            f'data/GOLD_{datetime.datetime.now().strftime("%Y%m%d")}.pkl')
    ...
    def gold_now(self):
        ...
        print(
            soup.find(
                class_="pull-left trailer text-info").text.replace(
                '\n',
                '').replace(
                "\r",
                ''),
        ...
        print(
            soup.find(
                "td",
                class_="rowSP_Ctrl_2_4_4 set-title-L-min-width-class").text.split("\r")[0])
        print(
            f'{soup.find("td", class_="rowSP_Ctrl_0_2_2 highlight text-center set-title-R-min-width-class").text}: {soup.find_all("td", class_="text-right")[0].text}')
        print(
            f'{soup.find("td", class_="rowSP_Ctrl_0_2_2 highlight text-center").text}: {soup.find_all("td", class_="text-right")[1].text}')

    def save_pkl(self, data):
        data.to_pickle(
            f'data/{self.currency}_{datetime.datetime.now().strftime("%Y%m%d")}.pkl')

    # 抓取能用的幣別
    def check_cur(self):
        soup = self._html("https://www.findrate.tw/currency.php")
        self.currencies = [x.split('href="/')[1].split('/')[0] for x in str(
            soup.find_all(class_="listbtns center")[0].find('li')).split('\n')[:-1]]
    ...
    def _currencies(self, show=True):
        soup = self._html("https://www.findrate.tw/currency.php")
        en_list = [x.split('href="/')[1].split('/')[0] for x in str(
            soup.find_all(class_="listbtns center")[0].find('li')).split('\n')[:-1]]
        zh_list = soup.find_all(class_="listbtns center")[
            0].find('li').text.split('\n')[:-1]
        cur_dict = {}
    ...
    def _onepage(self, year, page):
        ...
        if self.check:
            soup = self._html(
                f"https://historical.findrate.tw/his.php?c={self.currency}&year={year}&page={page}")
            ...
            for tr in trs:
                rows.append([td.text.replace('\n', '').replace(
                    '\xa0', '').replace(' ', '') for td in tr.find_all('td')])
    ...
    def _year(self, year):
        ...
        if self.check:
            ...
            web_data = web_data.sort_values('日期').reset_index()[
                page_data.columns]
    ...
    def news(self):
        ...
        self.news_data = pd.DataFrame(
            {
                'date': [
                    td.text for td in newslist.find_all("span")], 'title': [
                    td.text for td in newslist.find_all("h4")], 'context': [
                    td.text for td in newslist.find_all("p")]})
    ...
    def history(self):
        ...
        if self.check:
            ...
            if file and (
                file[0] == os.path.join(
                    'data',
                    f'{self.currency}_{datetime.datetime.now().strftime("%Y%m%d")}.pkl')):
            ...
            try:
                while start != end:
                    ...
                    self.his_data = pd.concat(
                        [self.his_data, tmp_data], axis=0, sort=False)
                    start += 1
    ...
    def plot(self, target_year=2010):
        ...
        if pd.isnull(self.his_data['即期買入']).mean() < 1:
            target_df = self.his_data[self.his_data['日期']
                                      >= pd.Timestamp(f'{target_year}-01-01')]
            plt.plot('日期', '即期買入', data=target_df, label='即期買入')
            plt.plot('日期', '即期賣出', data=target_df, label='即期賣出')
            plt.legend()
            plt.title(
                f'{self.cur_dict.get(self.currency)}匯率--最低買價:{target_df["即期賣出"].min()},最高賣價:{target_df["即期買入"].max()},最高報酬率:{round(target_df["即期買入"].max()/target_df["即期賣出"].min(),3)}')
            plt.xticks(rotation=45)
            plt.show()
        else:
...
if __name__ == '__main__':
    currency_list = NTex()._currencies()
    NTex().now_all()
